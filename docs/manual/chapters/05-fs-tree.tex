...
a fixture and almost cost the project's first defended scope; its
presence makes the parser correct on every FS tree dense enough to
grow past one leaf node.
...
An indexer that reproduces POSIX namespace plus logical and
allocated size needs five families of FS-tree records and one
xfield family.
...
Other families are decoded but not used for the indexer's emission
rows: \texttt{file\_extent} (chapter~13's extent-reference
territory), \texttt{snapshot\_metadata} and \texttt{snapshot\_name}
(deferred), and an ``unsupported'' bucket for record types the
parser does not recognise. A record falling into the unsupported
bucket on a volume that is otherwise in the allowlist triggers a
typed validation gap that the caller must consume.
