...
whole-machine scan that is otherwise silent for tens of
seconds. The in-process FFI consumed by the native shell of
chapter~13 does not pipe these events through today --- the
scan runs as one synchronous call --- but the event-emitting
machinery is preserved on the library side so a future
asynchronous-FFI slice can light up the stopwatch without
re-implementing it.
...
worker pool cleanly. The same event stream still flows from
the CLI binary; the in-process FFI used by the current native
shell awaits a slice that exposes it.}
\end{figure}
