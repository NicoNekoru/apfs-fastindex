...
serial path), so the user-visible stopwatch and entry counter
tick smoothly even on a whole-machine scan that is otherwise
silent for tens of seconds.

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/app-progress.png}
\caption{A whole-machine \texttt{/} scan mid-flight. The
stopwatch and entry counter come from the parallel walker's
progress thread; 231 paths have been recorded as skipped at this
point in the walk (sandboxed application data, sealed-system
locations). The Cancel control terminates the worker pool
cleanly.}
\end{figure}
