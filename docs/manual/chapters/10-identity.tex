...
Three things still have to land before a cache is part of the
product:

\begin{enumerate}
  \item Re-run EX-07's mutation grid against the native Rust full
    parse (now possible, since R1 is complete and the parallel
    fallback of chapter~11 has landed) to confirm the finding
    holds when the parser is the project's own production code
  ...
  \item Specify an invalidation policy that names every condition
    ...
    did not record (R2-A's allocated-size column is the most
    recent example of a column a pre-existing cache would not
    have recorded).
\end{enumerate}
