...
them in that order: logical first (the v1 anchor), allocated second
(R2-A, landed at the cost of one fail-closed case for sparse files).
...
The choice of logical size as the v1 anchor is deliberate. It has a
stable oracle (\texttt{st\_size}), applies uniformly to ordinary,
sparse, cloned, hard-linked, symlink, and compressed files, and is
the size every user-facing tool already agrees on.
...
The R2-A metric. Per inode, the number of bytes APFS has allocated
on disk for the file's content stream.

\textbf{Observation.} For ordinary, cloned, hard-linked, and symlink
cases the parser emits \texttt{j\_dstream\_t.alloced\_size} (or
zero for symlinks) and the result matches macOS's
\texttt{st\_blocks * 512} oracle. Two cases must fail closed in v1:
sparse files (where \texttt{alloced\_size} overstates the actual
allocation by the sparse-bytes hint) and decmpfs-compressed files
(where no on-disk field has been validated against the macOS
oracle).
...
\texttt{apfsck} sum-of-extents rule. The v1 emission rule
must therefore split the dstream case by presence of the
...
resource-fork-backed decmpfs path. v1 therefore refuses to emit a
value for compressed files. The \texttt{--summary} register lists
the column as \texttt{not\_claimed} for compressed rows and the
visualisation greys the metric toggle out for those rows.
...
\textbf{The v1 directory total is the \emph{unique-inode logical sum
...
\section{What V1 Still Does Not Promise}

Three metrics remain outside the v1 contract. Each has its own probe
plan and its own oracle; the open-questions chapter tracks their
status.
